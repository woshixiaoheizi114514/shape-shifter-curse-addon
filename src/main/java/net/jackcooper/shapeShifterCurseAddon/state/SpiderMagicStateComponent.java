...
/**
 * 魔法蜘蛛「织网术」模式开关持久化组件（服务端权威，自动同步到客户端）。
 *
 * <p>存 0=搭路 / 1=攻击。独立 CCA 组件，不挂 origin，因此<b>不会被 SSC 形态切换销毁重建</b>，
 * 天然跨会话 / 跨形态 / 死亡重生保留——解决原 apoli:resource 每次形态切换被重置回 start_value 的问题。
 * 重生复制策略 ALWAYS_COPY（见 RegSpiderMagicStateComponent）。
 */
public class SpiderMagicStateComponent implements AutoSyncedComponent {
