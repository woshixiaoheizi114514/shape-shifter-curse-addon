package net.jackcooper.shapeShifterCurseAddon.client.palette;
...
public class PalettePresetsScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("text.ssc_addon.palette.btn.to_editor"),
            b -> {
                if (this.parent instanceof net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorScreen) {
                    MinecraftClient.getInstance().setScreen(this.parent);
                } else {
                    MinecraftClient.getInstance().setScreen(
                            new net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorScreen(this.parent));
                }
    ...
    private void maybeContinuePresetTutorial(int botStartX, int closeY, int botBtnW) {
        ...
        if (!net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorScreen.PENDING_PRESET_TUTORIAL) return;
        net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorScreen.PENDING_PRESET_TUTORIAL = false;

        java.util.List<net.jackcooper.shapeShifterCurseAddon.client.colorpicker.ColorTutorialOverlay.TutorialStep> steps =
                new java.util.ArrayList<>();
        // 1. 同步范围（按世界 / 全局存储）
        steps.add(new net.jackcooper.shapeShifterCurseAddon.client.colorpicker.ColorTutorialOverlay.TutorialStep(
        ...
        this.client.setScreen(new net.jackcooper.shapeShifterCurseAddon.client.colorpicker.ColorTutorialOverlay(
                this, () -> {}, steps));
    }
    ...
    public void tick() {
        ...
        if (this.parent instanceof net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorScreen owner) {
            var working = owner.getWorkingForPreview();
            if (working != null) {
                net.jackcooper.shapeShifterCurseAddon.client.colorpicker.AdvancedColorBridge.applyPreview(working);
            }
        }
    }
